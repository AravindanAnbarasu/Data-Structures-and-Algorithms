...
public:
    ...
    int removeElement(std::vector<int>& nums, int val)
    {
        ...
        // 'i' acts as the "slow" pointer (keeps track of the position)
        int i = 0;

        // 'j' acts as the "fast" pointer (iterates through the entire array)
        int j = 0;

        while(j < nums.size())
        {
            if (nums[j] != val) // If current element is not the element to be removed
            {
                nums[i] = nums[j];
                i++;
            }
            // Always move the fast pointer forward to explore the entire array
            j++;
        }
        return i;
    }
